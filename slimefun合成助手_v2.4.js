...
function produce(item_name){
    ...
    if (workstation[product_method] == null) {
        Chat.log("无工作台："+product_method+", 未正确搭建/暂不支持/超出交互距离")
        return false
    }
...
var f = FS.open('./config/craft_formula.json.2')
...
if (reverse) {
    ...
    var ClickSlot_Listener = JsMacros.on("ClickSlot", JavaWrapper.methodToJavaAsync((event) => {
